...
| Cluster      | Program Id |
| :---        |    :----:   |
| **Localnet**     | `3W8Yph6SuCtJjWd2gHHsgy8UJ5FBYTiJBefd3n8oAqKG` |
| **Devnet**  | `3W8Yph6SuCtJjWd2gHHsgy8UJ5FBYTiJBefd3n8oAqKG` |
| **Mainnet**  | ``  |
...
## REPORT FIXES FROM MAGNUS

- u64 as price - need to fix
---- changed price to f32 at ln 35/49/68 in state.
---- converted LAMPORTS_PER_SOL to f32 then total price back to u64 during ix input

- admin fee logic
--- hardcoded fee & % in constants.rs

- anchor dependency upgrade
done

- check for token_balance after mint transfer to prevent re-entrancy bug -- reload the ctx after the transfer and assert the supply is correct
- spl-pod crate update
done
- spl-token-2022 crate update
done

- input sanitization on the the collection create
done -- do it for AiNFT too??
- access_control on lib.rs file, run before giving access to the lib ctx call


- check lamports/check balances after any cpi's
--- ln 148/182 check token balance before and after mint
--- transfer_nft check balancepost burn
...
<!-- fix checks pre/post mint-->
<!-- use a counter on the nf? -->
<!-- sanitize collection url -->
<!-- create fn for fetching dynamic image url -->